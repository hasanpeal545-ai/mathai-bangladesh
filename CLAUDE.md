...
- 🔄 Feature 1 — Dual Query Mode (reference-based + direct question) — backend FastAPI skeleton + `/api/health` done; `/api/chat` structure + validation done (dummy response, no RAG yet); `chapter`/`exercise` both optional in reference mode
...
- 🔄 **PDF Ingestion Pipeline** (`ingestion/pdf_extractor.py`, not a numbered spec feature) — OCR extraction ✅ (full 362-page book tested: 17 chapters, 38 exercises, 888 chunks). OCR+LLM dual-output/comparison system ✅ (tested on page 15). Admin preprocess/approve/reject/summary API endpoints ✅ built. Not yet done: actually pushing `approved/chunks.json` into Qdrant's `nctb_content` collection, and wiring "reference mode" retrieval in `/api/chat` to use any of this.
...
## Next Task
Awaiting user's permission — options include: pushing `approved/chunks.json` into Qdrant's `nctb_content` collection, running preprocessing on more pages/the other 6 books, refining OCR/LLM-fix quality further, or moving to a different feature (e.g. Feature 5). Per project rule: build one piece at a time, test it, then move on — no auto-advancing.

## Next Task
Waiting on the full-book OCR extraction background job to finish, then report chapter/exercise counts + 2 sample chunks to the user per their request, before asking permission for anything further (e.g. running this on the other 6 books, moving to Feature 5, or refining OCR quality). Per project rule: build one piece at a time, test it, then move on — no auto-advancing.
